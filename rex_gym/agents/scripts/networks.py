...
class GoToLinearGaussianPolicy(tf.contrib.rnn.RNNCell):
    """Indepent linear network with a tanh at the end for policy and feedforward network for the value.
  ...
  """
    ...
    def __call__(self, observation, state):
        ...
        with tf.variable_scope('value'):
            x = tf.contrib.layers.flatten(observation)
            for size in self._value_layers:
                x = tf.contrib.layers.fully_connected(x, size, tf.tanh)
            value = tf.contrib.layers.fully_connected(x, 1, None)[:, 0]
        return (mean, logstd, value), state
...
class SoftmaxPolicy(tf.contrib.rnn.RNNCell):
    """Independent linear network with a sigmoid at the end for policy and feedforward network for the value.
    Assumes that the action space is of size 1.
    """
