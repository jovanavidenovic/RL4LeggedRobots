...
class RexTrottingEnv(rex_gym_env.RexGymEnv):
  ...
  """
    ...
    """
        super(RexTrottingEnv,
              self).__init__(urdf_version=urdf_version,
                             ...
                             shake_weight=0.01,
                             distance_weight=1,)
        self._extension_amplitude = 0.225
        self._swing_amplitude = 0.2
        action_dim = 8
    ...
    def reset(self):
        ...
        super(RexTrottingEnv, self).reset(initial_motor_angles=self.init_pose, reset_duration=0.5)
        self.goal_reached = False
        self.is_terminating = False
        self._stay_still = False
    ...
    def _signal(self, t, action):
        ...
        trotting_signal = np.array([
            0, sw_second_pair + action[1], ext_second_pair + action[5], # front left angles
            0, sw_first_pair + action[0], ext_first_pair + action[4], # front right angles
            0, ext_first_pair + action[3], ext_first_pair + action[7], # rear left angles
            0, sw_second_pair + action[2], ext_second_pair + action[6], # rear right angles
        ])
        signal = trotting_signal
        return signal
...
# #         A numpy array of the reference leg positions.
# #         """
# #         # Generates the leg trajectories for the two diagonal pair of legs.
# #         ext_first_pair, sw_first_pair = self._gen_signal(t, 0)
# #         ext_second_pair, sw_second_pair = self._gen_signal(t, math.pi)
        
# #         trotting_signal = np.array([
# #             sw_second_pair + action[0], ext_second_pair + action[1], # front left angles
# #             sw_first_pair + action[2], ext_first_pair + action[3], # front right angles
# #             ext_first_pair + action[4], ext_first_pair + action[5], # rear left angles
# #             sw_second_pair + action[6], ext_second_pair + action[7], # rear right angles
# #         ])
# #         signal = trotting_signal
# #         return signal
...
# #             current_idx_i = 2 * i
# #             current_idx_j = 3 * i
# #             motor_pose[current_idx_j] = 0
# #             motor_pose[current_idx_j + 1] = leg_pose[current_idx_i + 1] + leg_pose[current_idx_i] # e + s
# #             motor_pose[current_idx_j + 2] = leg_pose[current_idx_i + 1] - leg_pose[current_idx_i] # e - s
# #         return motor_pose
...
#             current_idx = 3 * i
#             motor_pose[current_idx] = 0
#             motor_pose[current_idx + 1] = leg_pose[current_idx + 2] + leg_pose[current_idx+1] # e + s
#             motor_pose[current_idx + 2] = leg_pose[current_idx + 2] - leg_pose[current_idx+1] # e - s
#         return motor_pose
